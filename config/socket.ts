...
function setUpSocket(io: Server) {
  ...
  io.on('connection', (socket: Socket) => {
    socket.on('vote', async ({ pollOptionId, optionIndex, userEmail }) => {
      try {
        const updatedPollData:IPoll = await pollService.updateVote(pollOptionId,optionIndex,userEmail)
        socket.emit('voteSuccess');
        const userDoc = updatedPollData.votedUsers.find(user => user.email === userEmail);
  ...
  const addMessage: IChat = await chatService.addMessage(message, userEmail);
  const userName = addMessage.senderEmail.split("@")[0]
  socket.emit('receiveOwnMessage',{message,timestamp:addMessage.timestamp})
  socket.broadcast.emit("receiveMessage", {userName,message:addMessage.message,timestamp:addMessage.timestamp});
    });
