  ...
  try {
    ...
    messages.reverse().forEach(msg => {
      ...
      if (msg.senderEmail === email) {
        ...
        addMessageToDiv({ userName, message: msg.message, timestamp: msg.timestamp }, 'received', !initial);
      }
    });
...
socket.on('voteUpdate', (data) => {
  ...
  updatedPollData.options.forEach((option, index) => {
    const voteButtonElement = document.querySelector(`.vote-btn[data-option-id="${option._id}"][data-option-index="${index}"]`);
    if (voteButtonElement !== userDoc.optionIndex) {
      voteButtonElement.textContent = "Vote";
    }
    const voteCountElement = document.querySelector(`.vote-count[data-option-id="${option._id}"][data-option-index="${index}"]`);
    if (voteCountElement) {
    ...
    if (index === userDoc.optionIndex) {
      ...
      if (voteButtonElement) {
        voteButtonElement.textContent = 'Voted';  // Change button text to 'Voted'
      }
    }
  });
});
