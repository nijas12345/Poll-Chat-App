...
class PollService implements IPollService {
  ...
  userLogin = async (email: string, password: string): Promise<string> => {
    try {
      const existingUser: IUser | null =
        await this.pollRepository.findUserByEmail(email);
      if (existingUser) {
  ...
  updateVote = async (
    ...
    try {
      ...
      const pollData: IPoll | null = await this.pollRepository.findVoteByEmail(
        userEmail
      );
      if (!pollData) {
        ...
        const votedUser = pollData.votedUsers.find(
          (v) => v.email === userEmail
        );
